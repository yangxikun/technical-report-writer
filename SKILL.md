...
4. Organize the report around the reader's problem and decision:
...
5. Prefer one scenario carried through the report over unrelated examples. Use a second example only when it reveals a genuinely different boundary.
6. End important sections with a reusable principle, decision rule, or warning. Avoid slogans unsupported by the section.
7. Verify the artifact: factual consistency, valid links, matching TOC anchors and headings, responsive layout, balanced visual hierarchy, and valid HTML structure.
